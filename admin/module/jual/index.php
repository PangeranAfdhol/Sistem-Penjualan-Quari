      											...
      											if(!empty($_GET['nota'] == 'yes')) {
      												...
      												if(!empty($bayar))
      												{
      													...
      													if($bayar >= $total)
      													{
      														...
      														$sql_id_pesanan = "SELECT * FROM pesanan";
      														$row_id_pesanan = $config->prepare($sql_id_pesanan);
      														$row_id_pesanan->execute();
      														$hasil_id_pesanan = $row_id_pesanan->setFetchMode(PDO::FETCH_ASSOC);
      														$id_pesanan;
      														foreach($row_id_pesanan->fetchAll() as $k) {
      															$id_pesanan = $k['id_pesanan'];
      														}

      														for($x=0;$x<$jumlah_dipilih;$x++){

      															$d = array($id_barang[$x],$id_member[$x],$jumlah[$x],$total[$x],$tgl_input[$x],$periode[$x],$id_pesanan);
      															$sql = "INSERT INTO nota (id_barang,id_member,jumlah,total,tanggal_input,periode,id_pesanan) VALUES(?,?,?,?,?,?,?)";
															...
															// echo $total_stok;
      														...
      														$sql_update_id_pesanan = "UPDATE pesanan SET id_pesanan = ?";
      														$row_update_id_pesanan = $config->prepare($sql_update_id_pesanan);
      														$row_update_id_pesanan->execute(array($id_pesanan + 1));
